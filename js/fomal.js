...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | Gary_Corehex")[0];
}
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by Gary_Corehex %c 你正在访问Gary_Corehexの小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| Gary_Corehex") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('Gary_Corehex的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息",
